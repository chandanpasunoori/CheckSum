...
public class Jacksum {
    ...
    private boolean _f = false, _x = false, _X = false, _r = false, _t = false, _m = false,
            ...
            if (!_d || (_d && !Service.isSymbolicLink(file))) {
                ...
                if (list == null) {
                    ...
                    if (!_e && !_p && !_S) {
                        ...
                        if (_w && tmp.length() > workingdirlen) {
                            tmp = tmp.substring(workingdirlen);
                        } else if (_w && tmp.length() < workingdirlen) {
                            tmp = "";
                        }
    ...
    private String getChecksumOutput(String filename) throws IOException {
        ...
        if (_r && !_p) {
            ...
            if (_w) {
                filename = filename.substring(workingdirlen);
            }
            checksum.setFilename(filename);
        }
    ...
    private void expectation(AbstractChecksum checksum, String expected) throws ExitException {
        ...
        if (checksum.getEncoding().equalsIgnoreCase(AbstractChecksum.BASE64)
    ...
    private void expectationContinue(AbstractChecksum checksum, String expected) {
        String value = checksum.getFormattedValue();
        if (checksum.getEncoding().equalsIgnoreCase("base64")
                ? value.equals(expected) : value.equalsIgnoreCase(expected)) {
    ...
    public Jacksum(String args[]) throws ExitException {
                        ...
                        if (!verbosetmp.equals(DEFAULT)) {
                            ...
                            while (st.hasMoreTokens()) {
                                ...
                                if (s.equals("warnings")) {
                                    verbose.setWarnings(true);
                                } else if (s.equals("nowarnings")) {
                                    verbose.setWarnings(false);
                                } else if (s.equals("details")) {
                                    verbose.setDetails(true);
                                } else if (s.equals("nodetails")) {
                                    verbose.setDetails(false);
                                } else if (s.equals("summary")) {
                                    verbose.setSummary(true);
                                } else if (s.equals("nosummary")) {
                                    verbose.setSummary(false);
                                } else {
        ...
        if ((_o || _O) && (_u || _U) && outputFile.equals(errorFile)) {
            if (_m) {
                throw new ExitException("Jacksum: Error: stdout and stderr may not equal if -m is wanted.", ExitStatus.PARAMETER);
            }
            try {
...
//        }
        ...
        if (_m || _c) {
            ...
            if (_g && _G) {
                metaInfo.setGroupChar(checksum.getGroupChar());
            }
        ...
        if (_r || _D) {
            ...
            if (!f.exists()) {
                ...
                if (f.isDirectory()) {
                    ...
                    if (_w) {
                        workingdirlen = getWorkingdirLength(f.toString());
                    }

                    if (_r) {
                        recursDir(f.toString());
                    } else {
                        oneDir(f.toString());
                    }
                    if (_S) {
